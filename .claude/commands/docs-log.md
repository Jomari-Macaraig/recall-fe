...
3. **Organize by concept/file, not strictly by date.** Each meaningful piece of the app (a package, a config change, a route, a pattern like "Router + Query loader integration") gets its own section covering, thoroughly:
   ...
   - **Library API breakdown, whenever TanStack Router and/or TanStack Query APIs are involved.** Add a "TanStack Router pieces used" list and/or a "TanStack Query pieces used" list, each entry naming the specific export/hook/option (`createFileRoute`, `loader`, `context`, `useParams`, `queryOptions` shape, `queryClient.ensureQueryData`, `useSuspenseQuery`, etc.) with one or two sentences on what it does and, critically, **which library it's actually from** — the recurring point worth reinforcing every time is that Router owns the URL/navigation/loader-timing and Query owns fetching/caching/server-state, and a given file's code is usually a mix of both. Skip this list only for sections that genuinely involve neither library.
